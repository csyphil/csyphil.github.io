(function($){
    ...
    function trigger_resizable(isNoAnim) {
        ...
        if(!isMin && width > 767.98) {
            $('#mini-button').prop('checked', false);
            trigger_lsm_mini(isNoAnim);
            isMin = true;
        }
        else if(width < 767.98 && $('#sidebar').hasClass('mini-sidebar')) {
            $('#sidebar').removeClass('mini-sidebar');
            isMin = false;
        }
    }
    ...
    $('.hide-type-list .search-group input').on('click', function() {
        ...
        parent.children(".super-search-fm").attr(
            "action",
            $(this).val()
        );

        parent.find(".search-key").attr(
            "placeholder",
